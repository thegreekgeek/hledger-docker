...
WORKDIR /build
RUN git clone --depth 1 --branch 1.50.3 https://github.com/simonmichael/hledger && cd hledger && ls && stack install --verbose hledger hledger-ui hledger-web
RUN cd /build/hledger/bin && ./compile.sh && cp hledger* /root/.local/bin/

# Needs fixes for 1.42
RUN stack install --resolver=nightly-2025-11-19 hledger-stockquotes-0.1.3.2

# Needs fixes for multi-interval bug
# RUN stack install --resolver=$RESOLVER hledger-interest-1.6.7

# Needs fixes for 1.41
RUN stack install --resolver=nightly-2025-11-19 hledger-iadd-1.3.22
...
WORKDIR /build
RUN git clone https://github.com/adept/hledger-interest && cd hledger-interest && stack install --resolver=nightly-2025-11-19 hledger-interest && cd .. && rm -rf hledger-interest
