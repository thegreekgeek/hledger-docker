...
ENV RESOLVER=nightly-2025-12-04 \
    LC_ALL=C.UTF-8 \
...
WORKDIR /build
RUN git clone --depth 1 --branch 1.51.1 https://github.com/simonmichael/hledger && cd hledger && ls && stack install --verbose hledger hledger-ui hledger-web
...
RUN apt-get update && apt-get install --yes --no-install-recommends libgmp10 libtinfo6 sudo less jq curl python3 python3-venv git python3-pip && apt-get clean && rm -rf /var/lib/apt/lists
